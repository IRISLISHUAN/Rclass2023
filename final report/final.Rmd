...
<em>Amphiprion</em>: For the well-researched A. ocellaris,  the optimal temperature is said to be around 29℃ or 30℃ (Soman et al., 2021; Velasco et al., 2019).
...
Use two sample t-test and Fligner-Killeen test to test if it is significant difference between 2016 and 2019.
```{r}
#Two sample t-test
...
ggplot(Hypoplectrodes_2019_species, aes(x= species_name, y= species_number)) +
  ...
  theme(axis.text.x = element_text(vjust = 0.65))
```
We found that in the genus Hypoplectrodes, there was just one dominant species in 2016 and 2019.So it was hard to test the difference between two years. And then we changed the target genus from Hypoplectrodes to Lutjanus.
